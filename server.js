...
// REST API: Persistent Data Fetch for Dashboard Refresh
app.get('/api/records', (req, res) => {
    const currentDb = loadDatabase();
    res.json(currentDb.records || {});
});
...
function saveBase64ToFile(base64Data, defaultExt = 'jpeg') {
    ...
    try {
        ...
        if (match) ext = match[1] === 'jpeg' ? 'jpeg' : match[1];
...
io.on('connection', (socket) => {
    socket.on('send-document', (data) => {
        try {
            db = loadDatabase(); // Always take latest state
            db.lastToken += 1;
...
server.listen(PORT, () => console.log(`🚀 Hotel Desk Server active on http://localhost:${PORT}`));
